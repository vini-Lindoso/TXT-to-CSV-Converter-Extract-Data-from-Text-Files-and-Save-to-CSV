...
for numero_cliente in lista_clientes_desejados:  # Para cada cliente na lista de clientes desejados
    # Tenta encontrar a pasta do cliente que começa com o número do cliente
    pasta_cliente_encontrada = None
    
    for pasta in os.listdir(caminho_base):  # Lista todas as pastas no caminho base
        if pasta.startswith(numero_cliente):  # Verifica se o nome da pasta começa com o número do cliente
            pasta_cliente_encontrada = pasta
            break

    if pasta_cliente_encontrada:  # Se encontrar a pasta correspondente
        caminho_pasta_cliente = os.path.join(caminho_base, pasta_cliente_encontrada)  # Cria o caminho completo da pasta do cliente
        ...
        try:
            ...
            if os.path.isfile(caminho_arquivo_txt):
                ...
                with open(caminho_arquivo_txt, 'r') as arquivo_txt:
                    ...
                    if not linhas:  # Verifica se o arquivo está vazio
                        ...
                        for linha in linhas:
                            ...
                            elif any(palavra in linha_lower for palavra in ['senha:', 'senhas:', 'senha web']):  # Verifica se a linha contém 'Senha:' ou 'senha:'
                ...
                if senha or acesso or cnpj or login:
                    ...
                    print(f'  CNPJ: {cnpj}')
                    print(f'  Login: {login}')
                    print('-' * 30)  # Linha separadora para melhor visualização
                
            else:
                print(f'Arquivo {numero_cliente} não encontrado')  # Exibe mensagem de erro caso o arquivo não seja encontrado
                print('-' * 30)  # Linha separadora para melhor visualização
        ...
        print(f'Pasta para o cliente {numero_cliente} não encontrada')
        print('-' * 30)  # Linha separadora para melhor visualização
